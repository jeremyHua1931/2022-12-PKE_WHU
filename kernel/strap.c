...
static void handle_syscall(trapframe *tf) {
    ...
    tf->epc += 4;
    //TODO: lab1-1 思考题: 下一条指令, 栈+4
    //epc是异常返回地址寄存器,用户存储异常返回地址,系统调用后epc返回的是下一条指令的地址,在本操作系统中,每条指令有32位,也即4个字节,应该加4
 ...
 */
    ...
    //regs.a1存的是字符串的首地址
    long returnCode = do_syscall(tf->regs.a0, tf->regs.a1,
                                 tf->regs.a2, tf->regs.a3, tf->regs.a4,
